...
selection = st.selectbox("Select your predictor", ["XGBOOST", "Random Forest"])

if selection == "XGBOOST":
    st.write("You selected: {} model.".format("XGBOOST"))
    model = pickle.load(open('xgb_model', 'rb'))
else:
    st.write("You selected {} model.".format("Random Forest"))
...
km = st.sidebar.slider('What is the milage of your car?',0,100000,step = 500)
...
st.header("The configuration of your model is as follows:")
